import { GithubUserCache } from '../interfaces/github-user-cache.interface';

import { Redis } from '../lib/redis.lib';
import { StringEx } from '../lib/string-ex.lib';

export class GithubCacheUserDB {
  async get(key: string, pattern: string, target: string) {
    ...
    if (!(caches instanceof Error) && caches.length > 0) {
      const filter = caches
        .map((text) => StringEx.Decompress(text) as GithubUserCache)
  ...
  async set(key: string, cache: GithubUserCache): Promise<string | Error> {
